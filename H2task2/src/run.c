...
void 
initialize_structure(int *lattice, int ordered) {
    for (int i = 0; i < N; i++) {
...
double 
calculate_energy(int *lattice, int neighbors[N][NEIGHBORS]) {
    double energy = 0.0;
    for (int i = 0; i < N; i++) {
...
void 
create_neighbor_list(int neighbors[N][NEIGHBORS]) {
    int index = 0;
    for (int x = 0; x < L; x++) {
        for (int y = 0; y < L; y++) {
            for (int z = 0; z < L; z++) {
...
int 
metropolis_step(int *lattice, int neighbors[N][NEIGHBORS], double T) {
