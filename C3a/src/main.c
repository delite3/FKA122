...
void save_trajectory(FILE *file, double time, double *pos, double *vel, unsigned int ndims) {
    if (!file) {
        perror("File not open");
        return;
    }

    fprintf(file, "%10.5f ", time);
    for (int i = 0; i < ndims; i++) fprintf(file, "%10.5f ", pos[i]);
    for (int i = 0; i < ndims; i++) fprintf(file, "%10.5f ", vel[i]);
    fprintf(file, "\n");
}
...
solve_general(
        ...
        double *vel,  // Velocity vector. Initialize with init  ial velocity
        ...
        )
{
    ...
    FILE *file = fopen("trajectory.dat", "w");  // Open file to write output
    if (!file) {
    ...
    while(1) {
        ...
        // Save each time step's data to the file
        save_trajectory(file, time, pos, vel, ndims);
