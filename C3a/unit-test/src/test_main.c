...
{
    // We know the L2 norm of [3, 4] is 5.0
    double v[] = {3.0, 4.0};
    double expected_norm = 5.0;

    // Calculate the L2 norm of the vector
    double calculated_norm = l2_norm(v, 2);

    // Verify the calculated L2 norm is as expected
    ck_assert_double_eq_tol(calculated_norm, expected_norm, 1e-10);
}
